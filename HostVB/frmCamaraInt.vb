...
Public Class frmCamaraInt
    ...
    Dim ConfigFilePath As String = Path.Combine(My.Application.Info.DirectoryPath, CanonAPIbase.CONFIG_FILE)
    Dim lista1 As List(Of DataAccess.CHERFII)
    Dim lista2 As List(Of DataAccess.CHERFII)
    Dim dd As Integer
    Dim mm As Integer
    Dim aaaa As Integer
    Dim hr As Integer
    ...
    Dim Oper As Operacion = HostVB.Operacion.Interna
    ...
    Dim listaCHER As New List(Of CHERFII)
    Dim listaCHIT As New List(Of CHITCI)
    ...
    Sub Insertar()
        ...
        lectura.cabecera = lectura.PAQUETE
        lectura.cod_proceso = Oper
    ...
    Sub Grabar()
        Try
            ...
            dm.SaveChanges()
            ModificadoAttach()
            My.MyApplication.AbrirProceso(codBanco, Fecha, Moneda.CodMoneda, Oper, 1)
    ...
    Private Sub frmRecibido_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        ' grabar()
        '   Me.LecturasGridControl.MainView.SaveLayoutToXml(My.Computer.FileSystem.CombinePath(My.Computer.FileSystem.SpecialDirectories.AllUsersApplicationData, frmRecibido.GridConfigFile))
    ...
    Private Sub frmRecibido_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'If My.Computer.FileSystem.FileExists(My.Computer.FileSystem.CombinePath(My.Computer.FileSystem.SpecialDirectories.AllUsersApplicationData, frmRecibido.GridConfigFile)) Then
        '    Me.LecturasGridControl.ForceInitialize()
        '    Me.LecturasGridControl.MainView.RestoreLayoutFromXml(My.Computer.FileSystem.CombinePath(My.Computer.FileSystem.SpecialDirectories.AllUsersApplicationData, frmRecibido.GridConfigFile))
        'End If
        ...
        Me.CHERFIIBindingSource.DataSource = listaCHER
        Me.CHITCIBindingSource.DataSource = listaCHIT
    ...
    Public Property Paquete() As Integer
        ...
        Set(ByVal value As Integer)
            ...
            'Por si haya aumentado el lote manualmente
            'entonces usamos el valor manual
            If value <= Me.nudPaquete.Value Then
                Me.nudPaquete.Value += 1
                _Paquete = Me.nudPaquete.Value
            Else
                _Paquete = value
                Me.nudPaquete.Value = value
    ...
    Private Sub validarRepetidos()
        ...
        For i = 0 To LecturaView.RowCount - 1
            ...
            If listaRepetido.Count > 0 Then
                Me.LecturaView.SetColumnError(Me.colnumero_cheque, String.Format("El cheque esta repetido en fecha {0}, Maquina {1} ", listaRepetido(0).fecha_lectura.Value.ToShortDateString, listaRepetido(0).cod_maquina))
                '                r.Cells("tbcSerie").ErrorText = String.Format("El cheque esta repetido en fecha {0}, Maquina {1} ", listaRepetido(0).fecha_lectura.Value.ToShortDateString, listaRepetido(0).cod_maquina)
            End If
    ...
    Private Sub ToolStripButton6_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton6.Click
        Dim Archivo As String = Application.StartupPath & "\Lecturas.xls"
    ...
    Private Sub cmdImportar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdImportar.Click
        Dim frm As New frmCherfii()
        frm.Show()
        listaCHER.Clear()
    End Sub
    'Sub CargarDetalle_CHITCI(ByVal Cadena As String)
    ...
    Sub ConfigurarCREC()
        If Me.Oper = Operacion.Recibido Then
            ...
            Me.lnkValidarCHER.Visible = True
            Me.cmdImportar.Visible = True
            'Me.tfCREC.Visible = False
        Else
            Me.tfValidar.Height = 60
            Me.lnkValidarCHER.Visible = False
            Me.cmdImportar.Visible = False
            Me.tfCHERFII.Visible = False
            Me.TaskPane2.TaskFrames.Remove(Me.tfCHERFII)
        End If
    End Sub
    Private Sub lnkValidarCHER_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles lnkValidarCHER.LinkClicked
        ValidarCHERFII()
    End Sub
    Sub ValidarCHERFII(Optional ByVal Simple As Boolean = False)

        Dim ExisteError As Boolean = False
        'Dim lCherfii = Me.CHERFIIBindingSource
        ' listaCHER = Me.CHERFIIBindingSource.DataSource
        If listaCHER.Count = 0 Then
            If Not Simple Then
                MsgBox("Debe cargar primeramente el archivo CHERFII para realizar esta validacion")
            End If
            Return
        End If

        'Dim d = listaCHER(0).CtaDeposito.ToList()
        ...
        'dxCREC.Load(det)
        'Me.CHERFIIBindingSource.DataSource = Nothing
        'Me.CHERFIIBindingSource.DataSource = d
        ...
        For i = 0 To LecturaView.RowCount - 1
            ...
            If Not lc.EsCabecera Then
                Dim Existe = (From s In listaCHER Where s.CtaDeposito = lc.cuenta_cheque).FirstOrDefault
                If Existe Is Nothing Then
                    lc.AgregarError(15, "No se encuentra en el Archivo CHERFII.")
                    ExisteError = True
                Else
    ...
    Private Sub GridView1_CustomSummaryCalculate(ByVal sender As Object, ByVal e As DevExpress.Data.CustomSummaryEventArgs) Handles LecturaView.CustomSummaryCalculate
        ...
        If e.SummaryProcess = CustomSummaryProcess.Finalize Then
            Select Case summaryID
                Case 1
                    e.TotalValue = STCab
                    txtSubTotalCab.Text = STCab.ToString("N0")
    ...
    Private Sub GridView1_ValidateRow(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs) Handles LecturaView.ValidateRow
        ...
        Me.ValidarGridView(CType(e.Row, Lecturas))
    ...
    Sub ValidarGridView(ByVal lc As Lecturas)
        'Dim lc As Lecturas
        lc.Errores.Clear()
        lc.CORREGIDO = True
        If lc.monto <= 0 Then
    ...
    Private Sub chkLotesConError_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkLotesConError.CheckedChanged
    ...
    Private Sub LinkLabel5_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles LinkLabel5.LinkClicked
        ValidarCHITCI()
    End Sub
    Sub ValidarCHITCI(Optional ByVal Simple As Boolean = False)

        Dim ExisteError As Boolean = False
        'Dim lCherfii = Me.CHERFIIBindingSource
        ' listaCHER = Me.CHERFIIBindingSource.DataSource
        If listaCHIT.Count = 0 Then
            If Not Simple Then
                MsgBox("Debe cargar primeramente el archivo CHERFII para realizar esta validacion")
            End If
            Return
        ...
        For i = 0 To LecturaView.RowCount - 1
            ...
            If Not lc.EsCabecera Then
                Dim Existe = (From s In listaCHIT Where s.CtaDeposito = lc.cuenta_cheque).FirstOrDefault
                If Existe Is Nothing Then
                    lc.AgregarError(15, "No se encuentra en el Archivo CHITCI.")
                    ExisteError = True
                Else
                    ''Solo Aplicar la correccion cuando el monto es igual a 0
                    'If Existe.Cuenta = lc.cuenta_cheque And lc.monto = 0 And ChkCorregir.Checked Then
    ...
    Private Sub LecturasGridControl_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LecturasGridControl.Click
