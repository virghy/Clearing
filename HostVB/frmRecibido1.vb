...
Public Class frmRecibido1
    ...
    Private customers As New List(Of Lecturas)

    ' Declare a Customer object to store data for a row being edited.
    Private customerInEdit As Lecturas
    ...
    Sub New(ByVal Camara As String, ByVal Maquina As String, ByVal Operacion As Operacion)
        ...
        Me.cnro_lote.FilteringEnabled = True
        Me.ccod_banco_emisor.FilteringEnabled = True
        Me.ccod_banco_receptor.FilteringEnabled = True
        Me.cmonto.FilteringEnabled = True
        Me.cnumero_cheque.FilteringEnabled = True
        Me.ccuenta_cheque.FilteringEnabled = True
        Me.ccod_tipdoc.FilteringEnabled = True
        Me.cmonto.FilteringEnabled = True
        Me.ccod_sucursal.FilteringEnabled = True
        'Me..FilteringEnabled = True
        Me.cIdMotivo.FilteringEnabled = True
    ...
    Sub iniciarLecturaPrueba()
        ...
        data = "C3900E000000156523E03512752A050100000000149312B"    'Cheques con longitud mayot
        ...
        For Each det As Lecturas In lec
            ...
            If det.EsCabecera Then
                ...
                If Not det.cuenta_cheque Is Nothing Then
                    CuentaDeposito = det.cuenta_cheque
                End If
            Else
                cDetalle += 1
                ...
                If Not det.monto Is Nothing Then
                    tDetalle += det.monto
                End If
                If det.cuenta_deposito Is Nothing OrElse det.cuenta_deposito.TrimEnd <> CuentaDeposito Then
    ...
    Sub CargarDatos()
        ...
        Me.LecturasDataGridView.RowCount = lista.Count
    ...
    Private Sub LecturasDataGridView_CellValueNeeded(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellValueEventArgs) Handles LecturasDataGridView.CellValueNeeded
        If e.RowIndex = Me.LecturasDataGridView.RowCount - 1 Then
            Return
        End If
        ...
        Dim customerTmp As Lecturas = Nothing

        ' Store a reference to the Customer object for the row being painted.
        If e.RowIndex = rowInEdit Then
            customerTmp = Me.customerInEdit
        Else
            customerTmp = CType(Me.customers(e.RowIndex), Lecturas)
        End If

        ' Set the cell value to paint using the Customer object retrieved.
        Select Case Me.LecturasDataGridView.Columns(e.ColumnIndex).Name
            Case "cmonto"
                e.Value = customerTmp.monto
            Case "cnro_lote"
                e.Value = customerTmp.nro_lote
            Case "ccuenta_cheque"
                e.Value = customerTmp.cuenta_cheque
        End Select
    ...
    Private Sub frmRecibido_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ''Modo Virtual
        Me.LecturasDataGridView.DataSource = Nothing
        Me.LecturasDataGridView.VirtualMode = True
        Me.LecturasDataGridView.RowCount = 10
    ...
    Sub PonerTitulos()
        ...
        Me.cmonto.DefaultCellStyle.Format = "N" + Moneda.Decimales.ToString
        ...
        Me.cIdMotivo.Visible = (Oper = Operacion.Devuelto Or Oper = Operacion.Rechazado)
    End Sub
    ...
    Private Sub validar()
        ...
        For Each item As DataGridViewRow In LecturasDataGridView.Rows
            ...
            If Not lc.EsCabecera And (lc.cod_proceso = 1 Or lc.cod_proceso = 4) Then
                ...
                Dim ExisteCheque = (Aggregate l In dm.Lecturas _
                                    Into Any(l.numero_cheque = lc.numero_cheque _
                ...
                And l.cod_moneda = Moneda.CodMoneda And l.fecha_lectura = FechaAnterior And l.cod_proceso = procesoRechazadoDevuelto))
